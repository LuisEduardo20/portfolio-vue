...
export function useData() {
  ...
  const fetchEssentials = async () => {
    const jSections = await _loadJson(
      constants.BASE_PATH + "/data/sections.json"
    );
    const jSettings = await _loadJson(
      constants.BASE_PATH + "/data/settings.json"
    );
    const jStrings = await _loadJson(
      constants.BASE_PATH + "/data/strings.json"
    );
  ...
  const fetchAll = async () => {
    const jPlaces = await _loadJson(
      constants.BASE_PATH + "/data/info/places.json"
    );
    const jProfile = await _loadJson(
      constants.BASE_PATH + "/data/info/profile.json"
    );
  ...
  const _loadSections = async () => {
    for (const section of _jsonData.sections) {
      ...
      const sectionCategory = _findObjectWithId(
        _jsonData.categories,
        sectionCategoryId
      );
  ...
  const _resolveCrossJsonReference = (evalString) => {
    ...
    const targetObject =
      _jsonData[targetId] ||
      _findObjectWithId(_jsonData.sections, targetId)?.content;
    console.log("targetObject:", targetObject);

    if (!targetObject) {
      throw new Error(
        "Can't resolve cross json-reference. Target JSON doesn't exist: " +
          evalString
      );
    }
    ...
    for (const field of fields) {
      ...
      if (!resolvedObject) {
        throw new Error(
          "Can't resolve cross json-reference. Target property doesn't exist on specified JSON: " +
            evalString
        );
      }
    }
  ...
  const _updateLocalization = () => {
    ...
    if (!_localizationsCache[currentLanguageId]) {
      _localizationsCache[currentLanguageId] = _cacheLocalization(
        _jsonData,
        currentLanguageId
      );
    }
  ...
  const _cacheLocalization = (object, languageId) => {
    ...
    for (const [key, value] of Object.entries(object)) {
      if (Array.isArray(value)) {
        localizedFields[key] = value.map((valueItem) =>
          _cacheLocalization(valueItem, languageId)
        );
  ...
  const _getLocalizedEntries = (localesHash, languageId) => {
    ...
    for (let string in localesHash[defaultLanguageId]) {
      ...
      if (!translation) translation = localesHash[defaultLanguageId][string];
